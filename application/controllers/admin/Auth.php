		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					...
					if($result['admin_role_id']== 7){
						$this->session->set_flashdata('error', 'You are Caretaker ,Use App for Login');
						redirect(base_url('admin/auth/login'));
						exit();
					}
					...
					{ 
						...
						$this->session->set_userdata($admin_data);
						 ...
						 $this->rbac->set_access_in_session1(); // set access in session
                      
						redirect(base_url('admin/dashboard/'), 'refresh');
                       }
					   else
					   {
						$this->session->set_flashdata('error', 'Invalid role for login!');
						redirect(base_url('admin/auth/login'));
						exit();
					   }
